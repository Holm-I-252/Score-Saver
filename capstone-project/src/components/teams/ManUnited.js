...
class ManUnited extends Component {
  constructor() {
    super();
    this.state = {
      data: [],
  ...
  async getScore() {
    ...
    this.state.data.forEach((element) => {
      if (
        element.homeTeam.name === "Manchester United FC" ||
        element.awayTeam.name === "Manchester United FC"
      ) {
    ...
    scorers.data.scorers.forEach((element) => {
      if (element.team.name === "Manchester United FC") {
